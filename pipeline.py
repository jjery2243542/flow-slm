...
class GSLMPipeline(nn.Module):
    def __init__(self, conf, args):
        ...
        if self.conf.model.extra_future_tokens > 1 and self.conf.model.reduction_factor > 1:
        ...
        if self.conf.model.ssl_model == "mimi" and self.conf.optimizer.token_loss_weight > 0:
            ...
            aux_output_dim = self.ssl_model.model.config.codebook_size + n_special_tokens
            ...
            if hasattr(self.conf.model, "extra_future_tokens") and self.conf.model.extra_future_tokens > 0:
                aux_output_dim = aux_output_dim * (self.conf.model.extra_future_tokens * reduction_factor)
        else:
            aux_output_dim = None
        ...
        token_emb_dim = token_emb_dim * reduction_factor
        ...
        self.embed = nn.Embedding(3, embedding_dim=input_dim)
